...
import presenters.OpenNotePresenter;
...
public class OpenNoteActivity extends CreateNoteActivity
{	
	private ImageButton mImageButtonEditViewSwitch;
	
	private PresenterListener mPresenterListener;
	private OpenNotePresenter mPresenter;
	
	private GestureDetector mGestureDetector;
	private GestureHandler mGestureHandler = new GestureHandler ();
	
	protected OpenNotePresenter getPresenter ()
	{
		mPresenter = new OpenNotePresenter(getPresenterListener());
		return mPresenter;
	}
	...
	public void onBackPressed () { mPresenter.onBackPressed(null); }
	
	protected void setupPresenter ()
	{
		mPresenterListener = new PresenterListener();
		mPresenter = new OpenNotePresenter ((OpenNoteActivity.PresenterListener) mPresenterListener);
	}
	...
	protected class PresenterListener extends CreateNoteActivity.PresenterListener implements OpenNotePresenter.ViewNoteView
	{
		...
		public void showPreviousNoteImage () { showImage(R.drawable.arrow_to_the_left); }
		
		public void showNextNoteImage () { showImage(R.drawable.arrow_to_the_right); }
		
		public void showStopImage () { showImage(R.drawable.stop); }
		...
		public void finish () { OpenNoteActivity.this.finish(); }
		
		private void showImage (int imageID)
		{
			final ImageView image = new ImageView(OpenNoteActivity.this);
			image.setImageResource(imageID);
			RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams (RelativeLayout.LayoutParams.WRAP_CONTENT,
					  RelativeLayout.LayoutParams.WRAP_CONTENT);
			params.addRule (RelativeLayout.CENTER_IN_PARENT);
			image.setLayoutParams(params);
			mViewGroupRootLayout.addView(image);
			image.postDelayed(new Runnable()
			{
				@Override public void run() { mViewGroupRootLayout.removeView(image); } 
			}, 1000);
		}
	}
	...
	private class GestureHandler implements OnGestureListener
	{
		...
		public boolean onFling(MotionEvent start, MotionEvent finish, float velocityX, float velocityY)
		{
			...
			final ViewConfiguration vc = ViewConfiguration.get (OpenNoteActivity.this);
			final int minSwipeDistance = vc.getScaledTouchSlop ();
			
			OpenNotePresenter presenter = (OpenNotePresenter) mPresenter;
